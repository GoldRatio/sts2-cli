...
def show_native_save(save_path):
    ...
    if room:
        ...
        room_type_display = ROOM_TYPE_TEXT.get(room_type, room_type)
...
def desc(obj):
    ...
    if obj and isinstance(obj, str):
        ...
        def smart_replace(m):
            ...
            # Energy icons: {Energy:energyIcons()} → [Energy] Energy
...
def t(text):
    """Placeholder for UI strings (English only)."""
    return text
...
def card_description_display_lines(card):
    """Lines under the title row; [Prefix] merges into first line, then remaining loc lines."""
    cd_d = card_desc(card)
...
def combat_hand_inline_stat_str(stats, *, card=None, osty=None):
    """Title-row Dmg/Blk from RunSimulator ``stats`` (DynamicVars, keys lowercased).
    ...
    if show_deck:
        ...
        if cards:
            ...
            for cd in cards:
                ...
                print(f"    {n(cd['name'])}{up} ({cd.get('cost','?')}) {c(ctype_zh, 'dim')}{rare_part}{suf_part}")
...
def show_map(state, send_fn=None):
    ...
    for i, ch in enumerate(choices):
        ...
        ntype = NODE_TYPE_TEXT.get(ch["type"], ch["type"])
...
def show_rest_site(state):
    ...
    for opt in state.get("options", []):
        ...
        opt_name = REST_OPTIONS_TEXT.get(opt_id, opt_id)
...
def loc_resolve(key):
    """Resolve a loc key to readable text."""
    cache = _load_loc()
    ...
    for p in reversed(parts):
        if p not in ('title', 'description', 'options', 'pages', 'INITIAL'):
            name = cache.get(f"relics:{p}.title")
            d_val = cache.get(f"relics:{p}.description", "")
            if name:
                d = desc(d_val)
                return f"{name}" + (f" — {c(d, 'dim')}" if d else "")
...
def show_event(state):
    ...
    print(f"  {c(f'Event: {event_display}', 'bold')}")
...
""")
        ...
        if raw == "saves":
            ...
            if saves:
                ...
                print(f"  No saves found.")
            continue
        if raw == "quit":
            raise _QuitRequested()
        if raw == "abandon":
            confirm = input(f"  Abandon this run? (y/n): ")
...
def _quit_with_save(native_save_path, character, seed):
    ...
    print()
    try:
        ans = input(f"  Save before quitting? (y/n): ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        ans = "n"

    if ans not in ("y", "yes"):
        print(f"  Quitting without saving.")
        return None
...
def play(character="Ironclad", seed=None, auto=False, ascension=0, log=True,
    ...
    action_log = []
    print(f"  {c('Starting simulator...', 'dim')}")
    ...
    try:
        ...
        while True:
            ...
            if dec == "game_over":
                ...
                if auto:
                    ...
                    if choice == "help":
                        print(f"  Enter card index, e=end turn, p0=use potion 0")
                        continue
        ...
        while True:
            ...
            if result and result.get("type") == "save_error":
                ...
                try:
                    ans = input(f"  New path (Enter = quit without saving): ").strip()
                except (EOFError, KeyboardInterrupt):
                    ans = ""
...
if __name__ == "__main__":
    ...
    if args.saves:
        ...
        if saves:
            ...
            print(f"{'─' * 50}")
            print(f"  Replay saves: python3 play.py --load saves/<file>")
            print(f"  Native saves: python3 play.py --continue saves/<file>")
        else:
